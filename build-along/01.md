...
I manually filter the file produced by ChatGPT. This might all be a doomed
exercise, because I don't know how accurate ChatGPT was - there was a lot of
stuff there that I had to clean up, and worse, I don't know what are the parts
it messed up or did not even emit.

It gives us some sort of a base, though at 2700 lines, it's not a small one.

Now let us try defining those mappings again, this time on the smaller file.

1. Define a new CSS var `--mast-base-color` in the root element.
2. Search replace "#282c37" with `var(--mast-base-color) !important`.
3. Ditto, replacing "#9baec8" with `var(--mast-primary-color) !important`.
4. Ditto, replacing "#d9e1e8" with `var(--mast-secondary-color) !important`.
5. Ditto, replacing "#6364ff" with `var(--mast-highlight-color) !important`.

Now manually go through the remaining hardcoded values, and try to match them to
the various `lighten(...)` and `darken(...)` SCSS values. For each such unique
color, create a new CSS variable and add it to root.

It is not looking good, I'm spotting many small errors. This'll require a lot of
cleanup.

Add !important to all colors. Now, with trepidation, try out our resultant
`template.css` on our test Mastodon instance running in the codespace.

Thankfully, it just looks like the default Mastodon theme, which is great, since
that means we haven't completely broken everything.

As a sanity check, change the value of `--mast-base-color` in `:root` to "red"
and reload (Safari caches the CSS, needs a Opt - Cmd - R). It changes, alright,
we may have a way forward indeed.
