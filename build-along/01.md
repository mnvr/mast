...
1. Define a new CSS var `--mast-classic-base-color` in the root element.
2. Search replace "#282c37" with `var(--mast-classic-base-color)`.
3. Ditto, replacing "#9baec8" with `var(--mast-classic-primary-color)`.
4. Ditto, replacing "#d9e1e8" with `var(--mast-classic-secondary-color)`.
5. Ditto, replacing "#6364ff" with `var(--mast-classic-highlight-color)`.
